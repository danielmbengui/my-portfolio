...
<IconButton
    ...
    //ref={avatarRef}
      ...
      variant="dot"
    >
      <Avatar src="/me-no-back.png" sx={{ width: 100, height: 100, background:'var(--primary)' }} />
    </StyledBadge>
      </IconButton>

      <Typography>{`Salut 👋, je suis Daan !`}</Typography>
      <Typography>{`Bienvenue sur mon site internet !`}</Typography>
    <Stack direction={'row'}>
        <Link href={PAGE_LINK_RESUME} target={'_blank'}>
        <Button variant='contained'>{`Mon CV`}</Button>
        </Link>
        <Link href={PAGE_LINK_CHAT_BOT} target={'_blank'}>
        <Button>{`Discuter avec moi`}</Button>
        </Link>
    </Stack>
    </Stack>
</Box>
